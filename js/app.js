...
getMain.addEventListener("click", async (e) => {
    if (e.target.classList.contains("delete-icon")) {
        ...
        window.location = "https://compassionate-volhard-4a1844.netlify.app/html/editNews.html"
        ...
        if (!bookmarkArr.includes(e.target.id))
...
async function printAllNews(sort) {
    ...
    if (result.length > 0) {
        result.forEach(({ _id, headline, author, description, date, thumbnail }) => {
